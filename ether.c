...
ether_dump(const uint8_t *frame, size_t flen) {
    ...
    flockfile(stderr);
    fprintf(stderr, "ether: dst=%s src=%s type=0x%04x\n", ether_addr_ntop(hdr->dst, addr, sizeof(addr)), ether_addr_ntop(hdr->src, addr, sizeof(addr)), ntoh16(hdr->type));
    #ifdef HEXDUMP
...
{
    int8_t frame[ETHER_FRAME_SIZE_MAX] = {};
    struct ether_hdr *hdr = (struct ether_hdr *)frame;
    size_t flen, pad = 0;
    ...
    debugf("dev=%s type=0x%04x dst=%s len=%zu\n", dev->name, type, ether_addr_ntop(dst, NULL, 0), len);
...
int ether_input_helper(struct net_device *dev, ether_input_func_t callback) {
    ...
    debugf("dev=%s type=0x%04x src=%s len=%zu\n", dev->name, type, ether_addr_ntop(hdr->src, NULL, 0), flen);
    ether_dump(frame, flen);
