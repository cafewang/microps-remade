...
loopback_transmit(struct net_device *dev, uint16_t type, const uint8_t *data, size_t len, const void *dst) {
    ...
    if (PRIV(dev)->queue.num >= LOOPBACK_QUEUE_LIMIT) {
        ...
        errorf("loopback queue full, dropping packet");
        return -1;
    }
    ...
    mutex_unlock(&PRIV(dev)->mutex);
    debugf("packet queued, dev=%s, type=0x%04x, len=%zu, queue_num=%u", dev->name, type, len, num);
...
loopback_isr(unsigned int irq, void *id) {
    ...
    for (;;) {
        ...
        debugf("queue poped, dev=%s, type=0x%04x, len=%zu", dev->name, entry->type, entry->len);
        debugdump(entry->data, entry->len);
...
loopback_init(void) {
    ...
    struct loopback *lo = memory_alloc(sizeof(*lo));
    if (!lo) {
        errorf("memory_alloc() failure");
        return NULL;
    }
    ...
    if (intr_request_irq(lo->irq, loopback_isr, 0, "loopback", dev)) {
