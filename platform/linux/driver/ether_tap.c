...
ether_tap_open(struct net_device *dev) {
    ...
    if (tap->fd == -1) {
        errorf("open %s: %s", CLONE_DEVICE, strerror(errno));
        return -1;
    }
    ...
    /* set async io signal destination */
...
ether_tap_init(const char *name, const char *addr) {
    ...
    if (addr) {
        if (ether_addr_pton(addr, dev->addr) == -1) {
            errorf("ether_addr_pton failed, addr=%s", addr);
            net_device_free(dev);
            return NULL;
        }
    }
    ...
    intr_request_irq(tap->irq, ether_tap_isr, INTR_IRQ_SHARED, "ether_tap", dev);
    infof("registered, dev=%s, name=%s, addr=%s", dev->name, name, ether_addr_ntop(dev->addr, NULL, 0));
    return dev;
}
